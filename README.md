...
- 브랜드 공식 이벤트: 할리스, 빽다방, 컴포즈커피, 메가MGC커피, 공차, 파리바게뜨, 폴바셋, 스타벅스의 공식 이벤트 목록에서 할인/쿠폰/적립/무료/1+1 성격의 현재 또는 최근 이벤트를 추출
- 이디야: 공식 이벤트 목록 최신성이 낮아 우선 제외
- 투썸플레이스: 공식 목록이 AJAX로 로드되며 현재 공개 JSON 요청은 403 응답이라 우선 보류
